...
export default function CoursePage() {
    ...
    const courseTasks = tasks.filter((t) => t.userId === user?.id && t.courseId === courseId && !t.completed);
    ...
    if (!course) {
        return (
            <div className="text-center py-20">
                ...
                <Link to="/" className="text-blue-500 text-sm mt-2 inline-block">← Back to Home</Link>
            </div>
        );
    }
                ...
                {newNoteId === topic.id && (
                    <div className="ml-10 mb-2">
                        ...
                        <div className="flex gap-2 mt-1">
                            <button onClick={() => handleSaveNote(topic.id)} className="text-xs text-blue-500 font-medium">Save</button>
                            <button onClick={() => setNewNoteId(null)} className="text-xs text-slate-400">Cancel</button>
                        </div>
                    </div>
                )}
                            ...
                            {newSubtopicParent === topic.id && (
                                <div className="flex items-center gap-2 ml-6 py-1.5 px-3">
                                    <input
                                        ...
                                        autoFocus
                                    />
                                    <button onClick={() => handleAddSubtopic(topic.id)} className="text-xs text-blue-500 font-medium">Add</button>
        ...
        <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-0.5">{courseTasks.length} active tasks · {totalItems} study items</p>
                </div>
            </div>
            ...
            {totalItems > 0 && (
                <div className="card mb-6">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-[14px] font-bold text-[#111827]">{course.courseName} Progress: {progress}%</span>
                    </div>
            ...
            <div className="mb-4">
                <div className="flex items-center justify-between mb-3">
                    <h2 className="text-[15px] font-bold text-[#111827]">Study Roadmap</h2>
                </div>
                ...
                {rootTopics.length === 0 ? (
                    <div className="text-center py-12 card bg-slate-50/50 border-dashed border-2">
                        ...
                        <p className="text-slate-500 font-medium mb-1">Build your study roadmap</p>
                        <p className="text-slate-500 text-[13px]">Add chapters, topics, and checklists to track completion</p>
                    </div>
                ) : (
            ...
            <div className="flex items-center gap-2 mt-4 mt-2">
                <input
                    ...
                    placeholder="Add a new chapter or topic..."
                />
                <button onClick={handleAddTopic} className="btn-primary !py-3 !px-5 shadow-sm">Add</button>
            </div>
        </div>
    );
}
